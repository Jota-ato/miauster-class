...
export interface ILanguageLevelsRepository {
    getByLevelAndLanguage(levelId: string, languageId: string): Promise<LanguageLevel | null>
...
class LanguageLevelsRepository implements ILanguageLevelsRepository {
    ...
    async getByLevelAndLanguage(levelId: string, languageId: string): Promise<LanguageLevel | null> {
        return await db
            .query
            .languagesLevels
            .findFirst({
                where: (languageLevel, { and, eq }) => and(
